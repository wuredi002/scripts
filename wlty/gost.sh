...
# 文件路径
SCRIPT_PATH="/usr/local/bin/g"
LINK_PATH="/usr/local/bin/G"
ROOT_SCRIPT_PATH="/root/gost.sh"
# 检查是否已经设置过
if [ ! -f "$SCRIPT_PATH" ]; then
    # 复制脚本到 /usr/local/bin 并设置权限
    cp "$ROOT_SCRIPT_PATH" "$SCRIPT_PATH"
    chmod u+x "$SCRIPT_PATH"
    # 创建符号链接
    ln -s "$SCRIPT_PATH" "$LINK_PATH" > /dev/null 2>&1
    chmod u+x "$LINK_PATH"
fi
# 删除 root 目录下的脚本
rm -f "$ROOT_SCRIPT_PATH"
