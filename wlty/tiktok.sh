...
checkOS(){
    ...
    if [ -n "$ifCentOS" ];then
        ...
        if [[ "$OS_Version" -lt "5" ]];then
            echo -e "${Font_Red}此脚本不支持CentOS${OS_Version},请升级至CentOS7或9或更换其他操作系统${Font_Suffix}"
...
function Goodbye() {
    ...
    echo -e "${Font_Yellow}联系咨询微信: Falltoher-1314${Font_Suffix}"
    echo -e "================================================"
}
clear
function ScriptTitle() {
    echo -e "${Font_SkyBlue}【Tiktok区域检测】${Font_Suffix}"
    echo -e "${Font_Green}BUG反馈或使用交流可去网站${Font_Suffix} ${Font_Yellow} ${Font_Suffix}"
    echo ""
    echo -e " ** 测试时间: $(date)"
    echo ""
}
