...
function update_do(){
    ...
    echo "平台架构为${bit}"
    if [[ ${bit} == "arm64" ]]; then
        docker pull docker.xdmb.xyz/xiandan/release/arm:${vNum}
        docker run --platform linux/${bit} --log-opt max-size=10m --dns=${dns} --log-driver json-file --restart=always --name=xiandan -v /etc/xdzz:/xiandan -p ${port}:8080 -d docker.xdmb.xyz/xiandan/release/arm:${vNum}
    else
...
function update_do_bot {
    ...
    echo "平台架构为${bit}"
    if [[ ${bit} == "arm64" ]]; then
        docker pull docker.xdmb.xyz/xiandan/bot/arm:latest
        docker run --platform linux/${bit} --name xd-bot --network host --restart=always -v /etc/xdzz/bot_config:/usr/src/app/bot_config -d docker.xdmb.xyz/xiandan/bot/arm:latest
    else
        docker pull docker.xdmb.xyz/xiandan/bot:latest
